# NOTE: Cannot use DESeq2 for GeoMx data as multiple samples comes from same patient

proj <- "Neil_Spatial"
species <- "Homo sapiens"
contrasts <- c("Colon.Met-Colon.Pri",
               "Liver.TAdj-Liver.Dist",
               "Liver.TAdj-Colon.Stroma")
...
heatmap.override <- list(
  ...
  col.ann          = c("TissueStatus", "TissueOrigin", "TissueType", "CaseNumber"),                  # Column annotation
  #row.ann          = NULL,                  # Row annotation
  #col.gaps         = NULL,                  # Column gaps
  #row.gaps         = NULL,                  # Row gaps
  col.cluster      = c("TissueStatus"),                 # Column clustering
