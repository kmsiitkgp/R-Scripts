...
calc_stats <- function(data_mat, metadata, Target, Reference, quant_norm, file_suffix, results_path){
  ...
  for (j in 1:nrow(data_mat)){
    ...
    data_mat[j,] <- data[,colnames(data_mat[j,])] %>% unlist(use.names=FALSE)
  }
  data_mat <- data_mat %>% dplyr::mutate(across(.cols=everything(), .fns = as.numeric))
  
  # Perform quantile normalization
  if (quant_norm == TRUE){
    df_norm <- as.data.frame(preprocessCore::normalize.quantiles(as.matrix(data_mat)))
    rownames(df_norm) <- rownames(data_mat)
    colnames(df_norm) <- colnames(data_mat)
  } else {
    df_norm <- data_mat
  }
  ...
  openxlsx::writeData(wb, sheet = "results", x = result, rowNames = FALSE)
  openxlsx::addWorksheet(wb, sheetName = "imputed")
  openxlsx::writeData(wb, sheet = "imputed", x = data_mat, rowNames = FALSE)
  openxlsx::addWorksheet(wb, sheetName = "quant_norm")
  openxlsx::writeData(wb, sheet = "quant_norm", x = df_norm, rowNames = FALSE)
