...
### Get gene annotations from ENSEMBL & ENTREZ
# Output is a list of 2 dataframes: first for human & second for mouse with 
  ...
  for (species in c("Homo sapiens", "Mus musculus")){
    ...
    annotations <- dplyr::full_join(ensembl, entrez, by=c("ENSEMBL_ID"="ENSEMBL_ID")) %>%
      dplyr::select(ENSEMBL_ID, ENSEMBL_TRANSCRIPT, ENTREZ_ID, ENSEMBL_SYMBOL, 
                    ENTREZ_SYMBOL, ENSEMBL_BIOTYPE, ENTREZ_BIOTYPE, START, END, CHR, 
                    STRAND, DESCRIPTION)
...
### Reformat meta_data
# Column Sample.ID MUST be present
...
prep_metadata <- function(meta_data, read_data){  
  ...
  # # Keep ONLY data that is being compared and removed unwanted samples
  # index <- c()
  # for (l in length(DEG.params$Variable)){
  #   ind <- which(meta_data[,DEG.params$Variable[l]] %in% unique(c(DEG.params$Target, DEG.params$Reference)))
  #   index <- c(index, ind)
  # }
  # 
  ...
  # }
  # 
  # # Set appropriate reference levels for every parameter in design variable
  # # This DOESNT matter since we are using contrasts in results()
  # for (i in 1:length(DEG.params$design.ref)){
  #   meta_data[,stringr::str_split(string=DEG.params$design.ref[i], pattern=":")[[1]][1]] <- stats::relevel(x = meta_data[,stringr::str_split(string=DEG.params$design.ref[i], pattern=":")[[1]][1]],
  #                                                                                                          ref = stringr::str_split(string=DEG.params$design.ref[i], pattern=":")[[1]][2])
  # }
...
### Check read_data and meta_data for errors
...
check_data <- function(read_data, meta_data){  
  ...
  NA_sample_list <- c()
  if (sum(!is.na(DEG.params$Variable)) > 0){
    for (i in 1:length(DEG.params$Variable)){
      NA_sample_list <- c(NA_sample_list, which(is.na(meta_data[DEG.params$Variable[i]])))
    }
  }
...
### Plot PCA
plotPCA_DESeq2 <- function(meta_data, read_data, suffix, output_path){
  ...
  df <- dplyr::inner_join(meta_data,
                          pca$x %>% data.frame () %>% tibble::rownames_to_column("Sample.ID"),
                          by=c("Sample.ID"="Sample.ID"))   
  
  ggplot2::ggplot(df, aes(x=PC1, y=PC2, color=Comparisons)) + #get(DEG.params$Variable[1])
    ggplot2::geom_point(size=3, shape=16) +
    ggplot2::theme_light() +
    ggrepel::geom_text_repel(aes(label=Sample.ID),
                             show.legend = FALSE) +
    ggplot2::labs(color = "Comparisons", #DEG.params$Variable[1],
  ...
  # Save the PCA plot
  ggplot2::ggsave(filename=paste0("PCA.Plot.", suffix, ".tiff"),
                  plot=last_plot(),
                  device="jpeg",
                  path=output_path,
                  width=7,
...
### Plot MA
plotMA_DESeq2 <- function(dds, output_path){
  
  # The output of plotMA is not a ggplot2 object. So, we cant use ggsave()
  grDevices::tiff(filename=paste0(output_path, "MA_Plot.tiff"),
                  width=8.5,
                  height=11,
                  units="in",
                  bg="white",
...
plotDispEst_DESeq2 <- function(dds, suffix, output_path){  
  ...
  grDevices::jpeg(filename=paste0(output_path, "Dispersion_Plot_overall_", approach, "_", suffix, ".jpg"),
                  width=8.5,
                  height=11,
                  units=c("in"),
                  quality=75,
...
### Add gene symbols to normalized counts dataframe
# normalized_counts dataframe MUST have ID column
# Automatically, identify if ID column has human or mouse 
# gene symbols/ensembl_ids/entrez_ids & add gene SYMBOL
add_annotation <- function(normalized_counts){
  
  # Get annotations
  annotations <- get_annotations()
  
  # Figure out if ID columns has ensembl_id, entrez_id or gene symbols of human or mouse 
  items <- c(ensembl_id_human     = length(intersect(normalized_counts$ID, annotations[[1]]$ENSEMBL_ID)),
             entrez_id_human      = length(intersect(normalized_counts$ID, annotations[[1]]$ENTREZ_ID)),
             ensembl_symbol_human = length(intersect(normalized_counts$ID, annotations[[1]]$ENSEMBL_SYMBOL)),
             entrez_symbol_human  = length(intersect(normalized_counts$ID, annotations[[1]]$ENTREZ_SYMBOL)),
             ensembl_id_mouse     = length(intersect(normalized_counts$ID, annotations[[2]]$ENSEMBL_ID)),
             entrez_id_mouse      = length(intersect(normalized_counts$ID, annotations[[2]]$ENTREZ_ID)),
             ensembl_symbol_mouse = length(intersect(normalized_counts$ID, annotations[[2]]$ENSEMBL_SYMBOL)),
             entrez_symbol_mouse  = length(intersect(normalized_counts$ID, annotations[[2]]$ENTREZ_SYMBOL)))
  id <- names(items[which(items == max(items))])
  
  print(items)
  print(id)
  
  if (grepl(pattern = "human", x = id)){
    annotations <- annotations[[1]]
  } else if (grepl(pattern = "mouse", x = id)){
    annotations <- annotations[[2]]
  }
  
  if (grepl(pattern = "ensembl", x = id)){
    cols_to_keep <- c("ENSEMBL_ID", "ENSEMBL_SYMBOL")
    cols_to_remove <- setdiff(colnames(annotations), cols_to_keep)
  } else if (grepl(pattern = "entrez", x = id)){
    cols_to_keep <- c("ENTREZ_ID", "ENTREZ_SYMBOL")
    cols_to_remove <- setdiff(colnames(annotations), cols_to_keep)
  }
  
  if (grepl(pattern ="ensembl_id", x = id)){
  ...
  }  else if (grepl(pattern ="entrez_id", x = id)){
    normalized_counts <- annotations %>%
  ...
  }   else if (grepl(pattern ="ensembl_symbol", x = id)){
    normalized_counts <- annotations %>%
  ...
  }  else if (grepl(pattern ="entrez_symbol", x = id)){
    normalized_counts <- annotations %>%
...
run_deseq2 <- function(dds, meta_data, DEG.params, n, approach, output_path){  
  ...
  # NOTE: colData(dds) is identical to meta_data with sizeFactor column added
  ...
  #contraster <- contraster(dds, DEG.params$Target[n], DEG.params$Reference[n])
  
  # Calculate results
  res <- DESeq2::results(object               = dds,
                         contrast             = contrast,
                         lfcThreshold         = DEG.params$lfc.cutoff,
                         altHypothesis        = "greaterAbs",
                         cooksCutoff          = TRUE,
                         independentFiltering = TRUE,
                         alpha                = DEG.params$padj.cutoff,
                         pAdjustMethod        = "BH")
  ...
  # Add annotations
  results <- add_annotation(results)
  
  # Add unique identifier to each result file
  file_suffix <- DEG.params$contrast[n]
  #file_suffix <- paste(DEG.params$Target[n], "vs", DEG.params$Reference[n], sep='_')
  ...
  openxlsx::saveWorkbook(wb, file=paste0(output_path, "DEGs.", file_suffix , "_", approach, ".xlsx"), 
                         overwrite=TRUE)
...
# Output is a list with 3 dataframes of pathways with genes
pathway_analysis <- function(DEGs_df, gmt_files){
  ...
  #****************************************************************************#
  #        PREPARE A RANKED GENE LIST OF ALL GENES EXPRESSED FOR GSEA          #
  #****************************************************************************#
  ...
  for (gmt_file in gmt_files){
    ...
    # Run enricher()
    # NOTE: DO NOT USE clusterProfiler::enrichGO().
    ...
    if (!is.null(gsea)){
      gsea_results <- gsea@result %>% # you get more significant pathways from fgsea
        dplyr::filter(p.adjust <= 0.05)   # you get more significant pathways from fgsea
      gsea_df <- dplyr::bind_rows(gsea_df, gsea_results)
    }
    if (!is.null(ora)){
      ora_results <- ora@result %>%
        dplyr::filter(p.adjust <= 0.05)
      ora_df <- dplyr::bind_rows(ora_df, ora_results)
    }
  ...
  if (nrow(fgsea_df) > 0){
    ...
    fgsea_df <- fgsea_df %>%
      ...
      dplyr::rename(Description = pathway, geneID = leadingEdge) %>%
      dplyr::arrange(desc(abs_NES)) %>%
      dplyr::mutate(Direction = dplyr::if_else(NES > 0, "Upregulated", "Downregulated")) %>%
      tibble::remove_rownames() %>%
      dplyr::select(everything(), -geneID , geneID)
    
    # Split geneID into columns
    max_len <- max(unlist(lapply(X=stringr::str_split(string = fgsea_df$geneID, pattern = ","), FUN=length)))
    fgsea_df <- fgsea_df %>%
      tidyr::separate(col = geneID, into= paste0("gene", 1:max_len), sep=",", remove=TRUE, fill= "right") %>%
      dplyr::mutate(across(.cols = starts_with("gene"), ~ map_chr(.x , ~gsub("\"|\\)|c\\(", "",.x))))
    
    # Add a new column with tentative info of major pathways for easy investigation
    fgsea_df <- add_major_pathway(fgsea_df)
  }
  ...
  if (nrow(gsea_df) > 0){
    ...
    gsea_df <- gsea_df %>% 
      ...
      dplyr::mutate(Direction = dplyr::if_else(NES > 0, "Upregulated", "Downregulated")) %>%
      tibble::remove_rownames() %>%
      dplyr::select(everything(), -c("ID"), -geneID , geneID)
    
    # Split geneID into columns
    max_len <- max(unlist(lapply(X=stringr::str_split(string = gsea_df$geneID, pattern = "/"), FUN=length)))
    gsea_df <- gsea_df %>%
      tidyr::separate(col = geneID, into= paste0("gene", 1:max_len), sep="/", remove=TRUE, fill= "right") %>%
      dplyr::mutate(across(.cols = starts_with("gene"), ~ map_chr(.x , ~gsub("\"|\\)|c\\(", "",.x))))
    
    # Add a new column with tentative info of major pathways for easy investigation
    gsea_df <- add_major_pathway(gsea_df)
  }
  
  if (nrow(ora_df) > 0){
    # Format results to make it plottable
    ora_df <- ora_df %>% 
      tidyr::separate(col = "GeneRatio", into = c("DEGs.pathway", "DEGs.collection")) %>%
      tidyr::separate(col = "BgRatio", into = c("genes.pathway", "genes.collection")) %>%
      dplyr::mutate_at(c("DEGs.pathway", "DEGs.collection", "genes.pathway", "genes.collection"), as.numeric) %>%
      dplyr::mutate(k.K = DEGs.pathway/DEGs.collection) %>%
      dplyr::arrange(desc(DEGs.pathway)) %>%
      tibble::remove_rownames() %>%
      dplyr::select(everything(), -c("ID"), -geneID , geneID)
    
    # Split geneID into columns
    max_len <- max(unlist(lapply(X=stringr::str_split(string = ora_df$geneID, pattern = "/"), FUN=length)))
    ora_df <- ora_df %>%
      tidyr::separate(col = geneID, into= paste0("gene", 1:max_len), sep="/", remove=TRUE, fill= "right") %>%
      dplyr::mutate(across(.cols = starts_with("gene"), ~ map_chr(.x , ~gsub("\"|\\)|c\\(", "",.x))))
    
    # Add a new column with tentative info of major pathways for easy investigation
    ora_df <- add_major_pathway(ora_df)
  }
  
  l <- list(fgsea_df, gsea_df, ora_df)
  return(l)
} 
...
#' Plot and Save a Volcano Plot for Differential Expression Results
#'
#' Generates a volcano plot from a differential expression data frame. Points are colored by direction 
#' of regulation and statistical significance. Top genes can be labeled or highlighted. The plot is 
#' saved as a TIFF file.
#'
#' @param volcano_df A data frame containing differential expression results. Must include columns:
#'   `log2FoldChange`, `padj`, and `SYMBOL`.
#' @param volcano_params A named list containing plotting parameters. Must include:
#'   \itemize{
#'     \item \code{padj.cutoff}: Adjusted p-value cutoff (e.g., 0.05)
#'     \item \code{lfc.cutoff}: Log2 fold-change cutoff (e.g., 1)
#'     \item \code{Target}: Group name for up-regulated genes
#'     \item \code{Reference}: Group name for down-regulated genes
#'     \item \code{plot.title}: Title for the plot
#'     \item \code{matrix_color}: A string referencing a named color vector in the environment
#'     \item \code{color_disp_genes}: Logical; if TRUE, highlight displayed genes in red
#'     \item \code{label_disp_genes}: Logical; if TRUE, label displayed genes using ggrepel
#'     \item \code{label_top}: Logical; if TRUE and \code{disp_genes} is empty, automatically label top 5 up/down genes
#'   }
#' @param disp_genes A character vector of gene symbols to highlight and/or label on the plot.
#'   If empty and \code{label_top = TRUE}, the top 5 up- and down-regulated genes will be used.
#' @param file_suffix A string used to name the saved output file.
#' @param output_path Directory path to save the output TIFF file.
#'
#' @return Invisibly returns the ggplot object. Side effect: saves TIFF plot to `output_path`.
#' 
#' @export 
plot_volcano <- function(volcano_df, volcano_params, disp_genes, file_suffix, output_path){
  ...
  if (!all(required_cols %in% colnames(volcano_df))) {
    stop("volcano_df must contain columns: log2FoldChange, padj, SYMBOL")
  }
  if (!dir.exists(output_path)) {
    stop("output_path does not exist.")
  }
  
  #--- Categorize direction and significance ---
  volcano_df <- volcano_df %>%
    dplyr::mutate(Direction = dplyr::case_when(padj < volcano_params$padj.cutoff & log2FoldChange > volcano_params$lfc.cutoff ~ paste0("Up in ", volcano_params$Target),
                  ...
                  padj = dplyr::case_when(is.na(padj) ~ 0,
                                          TRUE ~ padj),
                  Significance = dplyr::case_when(abs(log2FoldChange) >= volcano_params$lfc.cutoff & padj <= 0.001 ~ "FDR < 0.001",
                                                  abs(log2FoldChange) >= volcano_params$lfc.cutoff & padj <= 0.01  ~ "FDR < 0.01",
                                                  abs(log2FoldChange) >= volcano_params$lfc.cutoff & padj <= 0.05  ~ "FDR < 0.05",
                                                  TRUE ~ "Not Significant")) %>%
    dplyr::filter(!(padj == 0 & Direction == "Not Significant"))
  
  #--- Define axis limits ---
  x_lims <- quantile(volcano_df$log2FoldChange, probs = c(0.0001, 0.9999), na.rm = TRUE)
  x_full <- quantile(volcano_df$log2FoldChange, probs = c(0, 1), na.rm = TRUE)
  x_max <- dplyr::if_else(x_full[2] - x_lims[2] < 3, x_full[2], x_lims[2])
  x_min <- dplyr::if_else(abs(x_full[1]) - abs(x_lims[1]) < 3, x_full[1], x_lims[1])
  
  y_vals <- -log10(volcano_df$padj[volcano_df$padj > 0])
  y_lim <- quantile(y_vals, na.rm = TRUE, probs = c(0,1))
  ...
  if (!exists(volcano_params$matrix_color)) {
    stop("Color palette not found in global environment: ", volcano_params$matrix_color)
  }
  volcano_palette <- get(volcano_params$matrix_color)
  names(volcano_palette) <- c("Not Significant", 
                              paste0("Up in ", volcano_params$Reference), 
                              paste0("Up in ", volcano_params$Target))
  ...
  #--- Auto-select top genes if disp_genes is empty ---
  if (length(disp_genes) == 0 & volcano_params$label_top == TRUE){
    disp_genes <- c(volcano_df %>% 
  ...
  #--- Build plot ---
  p <- ggplot2::ggplot(data = volcano_df, aes(x = log2FoldChange, y = -log10(padj), color = Direction)) +
    ggplot2::geom_point(size = 1.25, position=position_jitter(width = 0.05, height = 0.05)) +
    ggplot2::theme_classic() +
    my_theme +
    ggplot2::labs(x = expression("log"[2]*"FC"),
                  y = expression("-log"[10]*"padj"),
                  fill = "Direction",
                  title = volcano_params$plot.title) +
    ...
    coord_cartesian(xlim = c(floor(x_min), ceiling(x_max)),
                    ylim = c(0, ceiling(max(abs(y_lim))))) +
    # Define the axis tick marks
    scale_x_continuous(breaks = seq(floor(x_min), ceiling(x_max), 1)) +
    scale_y_continuous(breaks = seq(0, ceiling(max(abs(y_lim))), 10)) +
  ...
  #--- Add highlights or labels for disp_genes ---
  if (volcano_params$color_disp_genes == TRUE) {
    p <- p + ggplot2::geom_point(data = volcano_df %>% dplyr::filter(SYMBOL %in% disp_genes), color = "red")
  }
  if (volcano_params$label_disp_genes == TRUE) {
    p <- p + ggrepel::geom_text_repel(
      data = volcano_df %>% dplyr::filter(SYMBOL %in% disp_genes),
