...
for(i in samples){
  for (f in c("raw_feature_bc_matrix.h5")){
...
for(i in paste0(samples, ".raw")){
  ...
  sample_metadata <- sample_metadata %>% 
    dplyr::mutate(Cell = rownames(sample_metadata),  # no need to add sample name to barcodes like single cell since we work with only 1 sample at a time
                  Sample = orig.ident,
...
# of the assay. 2um bins are too small to contain 100 UMIs or 50 genes and the
# entire Spatial.002um assay will be removed. So, it is better to just use
# 8um and 16um bins for spatial analysis
...
for(i in paste0(samples, ".raw")){
  ...
  gene_cutoff <- 50           # reduced to 50 from 250 used for spatial
  umi_cutoff <- 100           # reduced to 100 from 500 used for spatial
  mito_cutoff <- 0.2
  ribo_cutoff <- 0.05
...
filtered_pre_emptydrops_seurat <- get(paste0(samples[1], ".raw"))

# This has all barcodes AFTER removal of empty droplets but WIHTOUT ANY QC
filt_seurat <- get(paste0(samples[1], ".filt"))

# Identify common barcodes between our filtering and Cellranger filtered matrices
common_bc <- intersect(rownames(filtered_pre_emptydrops_seurat@meta.data %>% dplyr::filter(Sample == samples[1])),
                       rownames(filt_seurat@meta.data %>% dplyr::filter(orig.ident == samples[1])))
...
for(i in samples){
  saveRDS(filtered_post_emptydrops_seurat, file=paste0(seurat_results, i,".rds"))
}
